    ...
    {% if group_count > 0 %}
      <section class="ingredients-group">
        ...
        {% if top_group == "חומרי גלם" %}
          <p>רכיבי בסיס שחוזרים במתכונים ודורשים בחירה, השריה, הכנה או שימוש נכון.</p>
        {% elsif top_group == "תבלינים ותערובות" %}
          <p>תבלינים ותערובות שמגדירים טעם, ריח וזהות משפחתית במנות.</p>
        {% elsif top_group == "רטבים וממרחים" %}
          <p>רטבים, ממרחים ותוספות שמחברים בין מתכונים ומשנים את האופי של המנה.</p>
        {% elsif top_group == "כבישה ותסיסה" %}
          <p>רכיבים ותהליכים שבהם זמן, מלח, חומציות או תרבית יוצרים טעם ומרקם.</p>
        {% elsif top_group == "טכניקות ובסיסים" %}
          <p>הכנות, שלבים ועקרונות עבודה שמשפיעים על עומק הטעם והצלחת המתכון.</p>
        {% elsif top_group == "בשר, דגים ונתחים" %}
          <p>חומרי גלם מן החי, נתחים וסוגי הכנה שחשוב להבין לפני הבישול.</p>
        {% elsif top_group == "כלים ומכשירים" %}
          <p>כלים, מכשירים ושיטות עבודה שמופיעים סביב המתכונים או ישמשו הרחבות עתידיות.</p>
        {% elsif top_group == "עקרונות בישול" %}
          <p>עקרונות כלליים שעוזרים להבין למה מתכון עובד ולא רק איך לבצע אותו.</p>
        {% elsif top_group == "אחר" %}
          <p>פריטים שעדיין לא שויכו לקבוצת ידע קבועה ודורשים מיון עתידי.</p>
        {% endif %}
